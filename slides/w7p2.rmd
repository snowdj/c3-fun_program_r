...
---
# Challenge
Create a `barplot` function that takes an optional `fill_var` argument, using
{ggplot2}, which fills the bars according to another variable.
...
```{r barplot-fun}
barplot <- function(data, x, y, fill_var = NULL) {
	
	p <- ggplot(data, aes(factor(!!enquo(x)), !!enquo(y))) 
	
	if(!is.null(fill)) {
		p <- p + geom_col(aes(fill = factor(!!enquo(fill_var))))
	}
	else {
		p <- p + geom_col()
	}
...
```{r barplot-example2}
barplot(diamonds, cut, price, clarity)
```
